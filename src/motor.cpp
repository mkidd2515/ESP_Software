...
Motor::Motor(PinName dirPin, PinName pwmPin, int period_us, PinName chA,
             PinName chB) noexcept
    : duty_cycle(0), pulse_count(0), period(period_us), dirOut(dirPin),
      pwm(pwmPin), quad(chA, chB, NC, 256) {
  pwm.period_us(period_us);
  pulse_sampler.attach(callback(this, &Motor::calc_speed),
                       std::chrono::microseconds(TICKER_PERIOD_US));
}

void Motor::calc_speed(void) noexcept {
  int current_pulses = quad.getPulses();
  int dPulses = current_pulses - pulse_count;
  const float pulses_per_second = static_cast<float>(dPulses) /
                                  (static_cast<float>(TICKER_PERIOD_US) * 1e-6);
...
float Motor::get_speed(void) const noexcept { return current_speed; }
